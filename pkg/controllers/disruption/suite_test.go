...
var _ = BeforeSuite(func() {
	...
	prov = provisioning.NewProvisioner(env.Client, recorder, cloudProvider, cluster, env.Clock, draController, launchBackoff)
...
var _ = Describe("Simulate Scheduling", func() {
	...
	It("should ensure that we do not duplicate capacity for disrupted nodes with provisioning", func() {
		...
		p := provisioning.NewProvisioner(hangCreateClient, recorder, cloudProvider, cluster, env.Clock, deviceallocation.NewController(hangCreateClient), launchBackoff)
