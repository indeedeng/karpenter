...
func recordFailedValidations(consolidationType string, candidates int, reason string) {
	FailedValidationsTotal.Add(float64(candidates), map[string]string{ConsolidationTypeLabel: consolidationType, FailureReasonLabel: reason})
}
...
func (c *ConsolidationValidator) isValid(ctx context.Context, cmd Command, validationPeriod time.Duration) error {
	...
	if err := c.validateCommand(ctx, cmd, validatedCandidates); err != nil {
		if IsValidationError(err) {
			recordFailedValidations(c.validationType, len(cmd.Candidates), validationFailureReason(err))
		}
...
func (e *EmptinessValidator) validateCandidates(ctx context.Context, candidates ...*Candidate) ([]*Candidate, error) {
	...
	if len(validatedCandidates) == 0 {
		recordFailedValidations(e.validationType, len(candidates), ValidationFailureReasonChurn)
	...
	// Candidates are filtered individually, so the returned error reports the reason the first candidate was rejected.
	var failureReason string
	reject := func(reason string) bool {
		recordFailedValidations(e.validationType, 1, reason)
		if failureReason == "" {
	...
	if valid := lo.Filter(validatedCandidates, func(cn *Candidate, _ int) bool {
		if e.cluster.IsNodeNominated(cn.ProviderID()) {
			return reject(ValidationFailureReasonNominated)
		}
		if disruptionBudgetMapping[cn.NodePool.Name] == 0 {
			return reject(ValidationFailureReasonBudget)
		}
...
func (c *ConsolidationValidator) validateCandidates(ctx context.Context, candidates ...*Candidate) ([]*Candidate, error) {
	...
	if len(validatedCandidates) != len(candidates) {
		recordFailedValidations(c.validationType, len(candidates), ValidationFailureReasonChurn)
	...
	for _, vc := range validatedCandidates {
		if c.cluster.IsNodeNominated(vc.ProviderID()) {
			recordFailedValidations(c.validationType, len(candidates), ValidationFailureReasonNominated)
		...
		if disruptionBudgetMapping[vc.NodePool.Name] == 0 {
			recordFailedValidations(c.validationType, len(candidates), ValidationFailureReasonBudget)
