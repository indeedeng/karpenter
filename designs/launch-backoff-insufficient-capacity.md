...
sequenceDiagram
    ...
    T-->>P: Admit decision + reservation ID
    P->>K: Create annotated NodeClaim
    L->>T: Bind reservation to NodeClaim UID
    L->>C: Create capacity
    alt Launch succeeds
        C-->>L: Resolved instance type, capacity type, and zone
        L->>T: Refund other keys; ramp landed key
    else Insufficient capacity
        C-->>L: ICE with rejected offering keys
        L->>T: Refund other keys; clamp rejected keys
    end
```
...
### Previous proposal: Per-offering windows plus a per-NodePool launch budget
...
- first failure creates a `burst=1` entry with no allowance before `nextRefill`;
- refill restores `remaining` from `burst`, not from a stale reservation count;
- repeated failure inside a closed window does not move `nextRefill`;
- success ramps only the landed key through `1 → 2 → 4 → 8 → unrestricted`, at most once
  per clean generation;
- every failure/success completion order produces the same result: failure dominates success
  from its own or an older epoch;
- deleting and recreating an entry gives it a new incarnation, so an old bound reservation
  cannot refund or ramp the new entry even when generation and epoch numbers match;
- pessimistic reservation debits every tracked candidate with allowance exactly once;
- an exhausted candidate does not veto a NodeClaim with an unrestricted or successfully
  reserved alternative;
- admission leaves the NodeClaim's instance-type, zone, capacity-type, and `minValues`
  requirements unchanged;
- candidate computation runs after truncation and matches the provider-available offerings
  compatible with the emitted NodeClaim;
- a partially fulfilled batched CreateFleet result is split per NodeClaim: requestors assigned
  an instance settle success, while unfulfilled requestors with only capacity errors settle
  attributed ICE;
- failed overrides accompanying a successful split result update the provider cache without
  clamping that successful NodeClaim's core reservation;
- success and attributed ICE refund non-consumed candidates;
- empty-key ICE clamps every saved candidate;
- duplicate and late settlement cannot over-credit, and missed settlement self-heals at
  refill;
- two NodeClaims whose only launchable tracked candidate is the same key cannot spend the
  same unit;
- a NodeClaim with a healthy on-demand candidate is admitted while a spot sibling is
  exhausted;
- shared offering keys across NodePools share allowance;
- the filter DeepCopies only affected instance types and runs before cached allocatable
  offerings are computed;
- retryable `CreateError` retains its reservation through retry;
- an ambiguous Kubernetes create persists a reservation annotation when the object exists;
  unbound reservations self-clean by TTL while bound reservations survive retry until
  terminal lifecycle settlement;
- omitted NodeClaim, deletion-before-launch, terminal failure, and reservation TTL cleanup
  release bookkeeping;
- static capacity keeps its broad requirements and releases unused `NodePoolState` count;
- disruption reserves every replacement atomically before cordon and never consumes during
  simulation; rejected batches roll back every debit and every pre-create failure releases
  the batch;
- all offering-blocked pods sleep, while affinity, oversized resources, and `minValues`
  failures still requeue immediately even when every offering is unavailable;
- soft topology constraints still relax and hard spread waits for the next refill;
- active-entry and probe counters cover refill, refund, and entry deletion;
- restart loses state without leaving any persisted debit.
