...
Batch dynamic drift replacements so large rollouts finish at the rate allowed by disruption budgets instead of one node at a time.
...
- About 990 drifted nodes remained eligible.
...
- no cluster-wide pending pods or pods from other deleting nodes.
...
The semantic change is intentional: if a candidate's pods fit on another live node, Drift creates replacement capacity instead of shrinking the cluster. During a rollout, this avoids moving pods onto nodes that may also be drifted. Consolidation can remove extra capacity later.
...
Sort candidates by drift time, oldest first, while keeping candidates with no reschedulable pods first. Cache read-only inputs once. For each candidate with budget, emit a delete command if it has no reschedulable pods; otherwise run a fresh replacement-only solve and emit a replace command if all pods schedule. Reserve shared constraints and decrement the local NodePool budget after each accepted command.
...
Pass the deadline into scheduler construction and `Solve`; checking only between candidates does not cap a single long simulation. Deadline expiry returns commands already accepted. A zero timeout, selected when `DriftReplacementBatching` is disabled, disables the deadline and stops after the first accepted command. Candidate-deletion races are skipped; other simulation errors fail the pass as they do today.

The batch-size cap is required because `Controller.disrupt` starts every returned command concurrently, and `StartCommand` creates replacement NodeClaims before adding the command to the rate-limited queue. The cap bounds this initial API burst. Revisit 100 after measuring launch and API-server load.

### Scheduling State

Cache the node snapshot, PDB limits, managed NodePools, instance types, and DaemonSet pods once per pass. Each candidate gets fresh volume requirements, topology, DRA allocation, reservation tracking, remaining NodePool resources, and a new scheduler. `Scheduler.Solve` mutates these values and cannot be reused without a complete reset.

Do not build the scheduler with an empty `stateNodes` slice. That prevents existing-node packing, but it also loses current NodePool usage, node-owned DRA slices, and parts of topology accounting. Instead, add a replacement-only scheduling mode that uses the cluster snapshot for accounting while excluding existing nodes as packing targets. The candidate must be treated as deleting so its own resources and DRA allocations can be replaced.

Accepted commands also update a batch ledger for NodePool limits, reserved offerings, DRA allocations, and replacement topology. Later simulations see those reservations but cannot use earlier replacements as packing targets. This is narrower than carrying a full simulated cluster between candidates.
...
Add a `V(1)` log when the deadline is reached with `candidates_evaluated`, `commands`, and `candidates_remaining`. Do not add command count as a metric label; it is an unbounded observation value.
...
Monitor `GetCandidatesWithTotals` and per-candidate replacement simulation duration during rollout. Confirm that scheduler construction and batch accounting are cheap enough to produce multiple commands within one minute. If command startup or API-server load is unsafe, disable `DriftReplacementBatching` to return immediately to one accepted command per pass.
